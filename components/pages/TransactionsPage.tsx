...
import { Transaction, NewTransaction, BankAccount, Card, TransactionSummary, TransactionDetails } from '../../types';
...
interface TransactionsPageProps {
    transactions?: Transaction[]; // Legacy transactions (optional for backward compatibility)
    summaries?: TransactionSummary[]; // Aggregated summaries
    onAddTransaction: (newTx: NewTransaction) => void;
    onUpdateTransaction: (tx: Transaction) => void;
    accounts: (BankAccount | Card)[];
    useAggregatedView?: boolean; // Toggle between legacy and aggregated view
...
const TransactionsPage: React.FC<TransactionsPageProps> = ({
    ...
    // Filter logic for legacy transactions
    const filteredTransactions = useMemo(() => {
        if (useAggregatedView) return [];
        return transactions.filter(tx =>
            tx.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
            tx.category.toLowerCase().includes(searchTerm.toLowerCase()) ||
            tx.amount.toString().includes(searchTerm)
        );
    }, [transactions, searchTerm, useAggregatedView]);

    // Filter logic for aggregated summaries
    const filteredSummaries = useMemo(() => {
        if (!useAggregatedView) return [];
        return summaries.filter(summary =>
            summary.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
            summary.category.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            summary.totalAmount.toString().includes(searchTerm)
        );
    }, [summaries, searchTerm, useAggregatedView]);
        ...
        <div className="p-8 space-y-8 min-h-screen">
            <header className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-neutral-500 dark:text-neutral-400 mt-1">
                        {useAggregatedView
                            ? 'Visualização agregada - uma linha por compra'
            ...
            <div className="bg-white dark:bg-neutral-900 p-6 rounded-2xl shadow-lg">
                ...
                <TransactionList
                    ...
                    mode={useAggregatedView ? 'aggregated' : 'legacy'}
                />
            </div>
