              ...
              <li key={tx.id} className="py-3 flex items-center space-x-4">
                  ...
                  {tx.type === 'installment' && tx.installment ? (
                    <div className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                      <span className="block">
                        {tx.installment.total}x de {formatCurrency(tx.amount)}
                      </span>
                      <span className="block text-neutral-400">
                        nesta fatura: parcela {tx.installment.current}/{tx.installment.total} ({formatCurrency(tx.amount)})
                      </span>
                    </div>
                  ) : (
                ...
                <div className="text-right">
                  <p className="font-mono font-bold text-red-600">
                    - {tx.type === 'installment' && tx.installment
                      ? formatCurrency(tx.amount * tx.installment.total) // Estimating total for display
